...
public class Main_24511 {
    public static void main(String[] args) throws Exception{
        ...
        for(int i = 0; i < size; i++) types[i] = Integer.parseInt(st.nextToken());

        MyLinkedDeck myDeck = new MyLinkedDeck();
        st = new StringTokenizer(br.readLine());
        for(int i = 0; i < size; i++)
            if(types[i] == 0) myDeck.addLast(Integer.parseInt(st.nextToken()));
            else st.nextToken();
        ...
        for(int i = 0; i < cnt; i++){
            ...
            if(i == 0) bw.write("" + myDeck.pollLast());
            else bw.write(" " + myDeck.pollLast());
        }
        bw.flush();
        bw.close();
    }
