...
public class Main_14889 {
    ...
    private static void findMin(int step){
        if(step < startTeam.length){
            for(int i = (step == 0)? 0 : startTeam[step - 1] + 1; i < table.length; i++){
                team[i] = true;
                startTeam[step++] = i;
                findMin(step--);
                team[i] = false;
            ...
            for(int i = 0; i < startTeam.length; i++){
                for(int j = 0; j < startTeam.length; j++){
                    start += table[startTeam[i]][startTeam[j]];
                    link += table[linkTeam[i]][linkTeam[j]];
                }
            }
            int differ = (start > link)? start - link : link - start;
